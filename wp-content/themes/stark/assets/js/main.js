jQuery(document).ready(function ($) {
    ...
    /* задаем начальный top для блоков меню */
    $(document).on('scroll',function(){
        var topPosition = 0;
        var topPositionDocument = +$(this).scrollTop();     
        var topMainBlock = +$('div.main-nav')[0].offsetTop; 
        var heightTopPanel = +$('.__middle')[0].clientHeight;
        if(topPositionDocument < (heightTopPanel + topMainBlock)){
            topPosition += (heightTopPanel + topMainBlock) - topPositionDocument ;
        }       
        $('.top-panel-header').css('top', topPosition);
    })
...
$("#contact").submit(function() {
        ...
        $.ajax({
            type: "POST",
            url: "wp-content/themes/stark/template-parts/mail.php",
            data: str,
            success: function(msg) {
                if(msg == 'OK') {
                    result = '<div class="ok">Спасибо. Ваша скидка 10%. Мы Вас ожидаем.</div>';
                    $("#fields").hide();
                    $("#submit").hide();
                }
                else {result = msg;}
